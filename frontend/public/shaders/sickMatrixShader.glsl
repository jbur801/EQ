...
const vec3 RUNECOLOUR =  vec3(0.3, 0.6, 0.50);
const vec3 FINALRUNECOLOUR = vec3(0.4, 0.6, 0.20);
const int BLOCK_SIZE = 10;  //in cells
const int BLOCK_GAP = 2;    //in cells

const float WALK_SPEED = 0. * XYCELL_SIZE;
                    ...
                    if (v >= 0.0 && v < 1.0) {
                        ...
                        if (char_hash.x >= 0.1 || c == 0.) {  //10% of missed symbols
                            ...
                            if (a > 0.) {
                                ...
                                vec3 col = (c == 0. ?RUNECOLOUR : FINALRUNECOLOUR) / attenuation;
                                float a1 = result.a;
                                result.a = a1 + (1. - a1) * a;
